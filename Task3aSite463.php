...
// Get data
if (($site215 = fopen("data_463.csv", "r")) !== FALSE) 
{
    while (($data = fgetcsv($site215, 1000, ",")) !== FALSE) 
    {
      ...
      if(strpos($data[1], $certaintime) !== false)
      {
        ...
        if(strpos($data[1], '2016') !== false)
        {
          $x = $x + 1;
          $runningtotaly1 = $runningtotaly1 + $data[4];
        }
        if(strpos($data[1], '2017') !== false)
        {
          $y = $y + 1;
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Time', 'NO Level'],
          [ 2015,      parseInt('<?php echo $runningtotaly0; ?>')],
          [ 2016,      parseInt('<?php echo $runningtotaly1; ?>')],
          [ 2017,      parseInt('<?php echo $runningtotaly2; ?>')],
          [ 2018,      parseInt('<?php echo $runningtotaly3; ?>')],
          [ 2019,      parseInt('<?php echo $runningtotaly4; ?>')]
        ]);

        var options = {
          title: 'Carbon Monoxide levels over time, Site 463',
          hAxis: {title: 'Years', minValue: 2015, maxValue: 2019},
          vAxis: {title: 'Carbon Monoxide Levels', minValue: 0, maxValue: 40},
          legend: 'none'
        };
